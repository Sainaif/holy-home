...
// ExportAllocationsCSV exports allocations for a specific bill
func (s *ExportService) ExportAllocationsCSV(ctx context.Context, billID primitive.ObjectID) ([]byte, error) {
	// Get bill
	...
	// Write header
	header := []string{"Bill Type", "Period", "Subject Type", "Subject ID", "Subject Email", "Amount (PLN)", "Units", "Method"}
	if err := writer.Write(header); err != nil {
		return nil, fmt.Errorf("failed to write CSV header: %w", err)
	}
	...
	for _, alloc := range allocations {
		amount, _ := utils.DecimalToFloat(alloc.AmountPLN)
		units, _ := utils.DecimalToFloat(alloc.Units)

		email := userMap[alloc.SubjectID]
		if email == "" {
			email = "-"
		}
		...
		row := []string{
			bill.Type,
			fmt.Sprintf("%s to %s", bill.PeriodStart.Format("2006-01-02"), bill.PeriodEnd.Format("2006-01-02")),
			alloc.SubjectType,
			alloc.SubjectID.Hex(),
			email,
			fmt.Sprintf("%.2f", amount),
